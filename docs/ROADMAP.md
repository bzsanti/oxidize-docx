...
- [~] `pipeline/classifier.rs` — `ClassifierPipeline` con `StyleResolver` + `NumberingResolver` + `TableBuilder` integrados; emite `Paragraph`/`Heading`/`ListItem`/`Table` en document order, propaga `current_heading`. Heading detection consulta `StyleResolver::resolve_paragraph` primero (outlineLvl canónico) y cae al name-match `"heading N"` como fallback. Numbering detection consulta `resolved_num_pr` (2026-06-30): el `numPr` se resuelve por cadena de estilos (`StyleResolver::resolve_paragraph(...).num_pr`), no solo el `numPr` directo del párrafo, así las listas con estilo "List Bullet"/"List Number" (numId en el estilo) emiten `ListItem`. `styles_xml` ahora captura `numPr` con `ilvl` ausente como nivel 0 (OOXML §17.3.1.19). Pendiente: gestionar `SectionBreak`, complex fields y drawings.
...
- [x] Integration: fixture DOCX real → `rag_chunks()` produce N chunks con texto exacto verificado (2026-06-30). Fixture `tests/fixtures/quarterly_report.docx` generada por python-docx (productor OOXML independiente: namespaces completos, `styles.xml` con Heading/`outlineLvl`, `numbering.xml` real, listas vía estilo "List Bullet"). Test `tests/document_rag.rs::rag_chunks_over_real_fixture_produces_exact_heading_scoped_chunks` asevera 2 chunks heading-scoped con `text`/`element_types`/`paragraph_indices`/`heading_context`/`token_estimate` exactos. **Bug cazado y resuelto:** las listas con numbering heredado del estilo (no `numPr` directo — patrón dominante en Word real) se degradaban a `Paragraph`; ahora emiten `ListItem`. Ver nota en Fase 3 classifier.
